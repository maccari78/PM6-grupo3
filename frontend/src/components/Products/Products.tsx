...
import ShowAndDeleteFilter, {
  IFilter,
} from "../ShowAndDeleteFilter/ShowAndDeleteFilter";
...
const convertFiltersToQueryParams = (filters: IFilter) => {
  const params: { [key: string]: string } = {};

  if (filters.selectedModel) {
    params.model = filters.selectedModel;
  }
  ...
  if (filters.selectedColors?.length! > 0) {
    params.color = filters.selectedColors!.join("");
  }

  if (filters.selectedMileage) {
    params.mileage = filters.selectedMileage;
  }
...
const Products: React.FC = () => {
  ...
  const fetchFilters = async () => {
    ...
    try {
      const queryParams = filters
        ? new URLSearchParams(convertFiltersToQueryParams(filters)).toString()
        : "";
  ...
  const deleteFilter = (filterType: keyof IFilter, value?: string) => {
    setFilters((prevFilters) => {
      if (!prevFilters) return null;

      let updatedFilters: IFilter = {
        selectedModel: prevFilters.selectedModel,
        selectedBrand: prevFilters.selectedBrand,
        selectedColors: prevFilters.selectedColors,
        selectedMileage: prevFilters.selectedMileage,
      };

      if (filterType === "selectedColors" && value) {
        const updatedColors =
          prevFilters.selectedColors?.filter((color) => color !== value) || [];
        updatedFilters = {
          ...prevFilters,
          selectedColors: updatedColors.length > 0 ? updatedColors : null,
        };
      } else {
        return {
          ...prevFilters,
